package com.nian.shortlink.gateway.filter;


import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONObject;
import com.nian.shortlink.gateway.config.Config;
import com.nian.shortlink.gateway.dto.GatewayErrorResult;
...
public class TokenValidateGatewayFilterFactory extends AbstractGatewayFilterFactory<Config> {
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            if (!isWhitePathList(requestPath, requestMethod, config.getWhitePathList())) {
            ...
            //如注册、登录等不需要用户token的直接放行
            return chain.filter(exchange);
        };
    }

    private boolean isWhitePathList(String requestPath, String requestMethod, List<String> whitePathList) {
        return (!CollectionUtils.isEmpty(whitePathList) && whitePathList.contains(requestPath))
